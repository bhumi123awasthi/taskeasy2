...
export default function WikiPage({ project: propProject }) {
  ...
  useEffect(() => {
    ...
    const fetchPages = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`${API_BASE}/projects/${encodeURIComponent(effectiveProjectId)}/wiki`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleSaveNewPage = async () => {
    ...
    try {
      ...
      await axios.post(`${API_BASE}/projects/${encodeURIComponent(targetProjectId)}/wiki`, {
